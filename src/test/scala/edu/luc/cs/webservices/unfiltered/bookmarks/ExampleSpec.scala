...
object ExampleSpec extends Specification with unfiltered.spec.jetty.Served {
  ...
  def setup = {
	...
	_.filter(new App(new InMemoryUserRepository))
  }
  ...
  "The example app" should {
	  ...
	  "find nothing that has not been created" in {
	 	val status = try {
	      http x (host / user2 as_str) { case (code, _, _, _) => code }
	 	} catch { case StatusCode(code, _) => code }
	 	status must_== 404
	  }
	  ...
	  "allow unauthenticated creation below root" in {
	 	val status = try {
	      http x ((host / user <<< "") as_str) { case (code, _, _, _) => code }
	 	} catch { case StatusCode(code, _) => code }
	 	status must_== 201
	  }
	  ...
	  "require authentication for deletion" in {
	 	val status = try {
	      http x ((host / user DELETE) as_str) { case (code, _, _, _) => code }
	 	} catch { case StatusCode(code, _) => code }
	 	status must_== 401
	  }
	  ...
	  "allow authenticated deletion" in {
	 	val status = try {
	      http x ((host / user DELETE) as_! (user, user) as_str) { case (code, _, _, _) => code }
	 	} catch { case StatusCode(code, _) => code }
	 	status must_== 204
	  }	  
  }
